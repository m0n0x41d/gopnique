...
repository-tests:
	ERROR_TRACKER_REPOSITORY_POSTGRES_URL="$(REPOSITORY_POSTGRES_URL)" go test -tags=integration ./internal/adapters/postgres -run 'TestPostgres(RepositoryContract|IssueShortIDConcurrency|RetentionPurgesScopedProjectData|QuotaRejectsBeforePersistence|UptimeMonitorWorkflow)' -count=1 -v
...
e2e:
	ERROR_TRACKER_E2E_POSTGRES_URL="$(E2E_POSTGRES_URL)" go test -tags=integration ./internal/e2e -run 'TestPostgres(M1M2E2E|RateLimitE2E|SecurityCSPE2E|MinidumpE2E|DebugFileSymbolicationE2E|UptimeMonitorE2E|ObservabilityAPIE2E)' -count=1 -v
