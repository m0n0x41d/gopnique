create table if not exists google_chat_destinations (
  id uuid primary key,
...
alter table google_chat_destinations
  add constraint google_chat_destinations_label_nonempty
  check (length(btrim(label)) > 0) not valid;

alter table google_chat_destinations
  validate constraint google_chat_destinations_label_nonempty;

alter table google_chat_destinations
  add constraint google_chat_destinations_url_nonempty
  check (length(btrim(url)) > 0) not valid;

alter table google_chat_destinations
  validate constraint google_chat_destinations_url_nonempty;
...
alter table alert_rule_actions
  ...
  check (provider in ('telegram', 'webhook', 'email', 'discord', 'google_chat')) not valid;
...
create index if not exists notification_intents_google_chat_claim_idx
  on notification_intents(provider, status, next_attempt_at, created_at)
  where provider = 'google_chat' and status in ('pending', 'failed');
