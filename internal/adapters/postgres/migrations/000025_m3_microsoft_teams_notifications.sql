create table if not exists teams_destinations (
  id uuid primary key,
...
alter table teams_destinations
  add constraint teams_destinations_label_nonempty
  check (length(btrim(label)) > 0) not valid;

alter table teams_destinations
  validate constraint teams_destinations_label_nonempty;

alter table teams_destinations
  add constraint teams_destinations_url_nonempty
  check (length(btrim(url)) > 0) not valid;

alter table teams_destinations
  validate constraint teams_destinations_url_nonempty;
...
alter table alert_rule_actions
  ...
  check (provider in ('telegram', 'webhook', 'email', 'discord', 'google_chat', 'ntfy', 'microsoft_teams')) not valid;
...
create index if not exists notification_intents_teams_claim_idx
  on notification_intents(provider, status, next_attempt_at, created_at)
  where provider = 'microsoft_teams' and status in ('pending', 'failed');
