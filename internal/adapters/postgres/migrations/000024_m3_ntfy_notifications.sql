create table if not exists ntfy_destinations (
  id uuid primary key,
  ...
  unique (project_id, url, topic)
);

alter table ntfy_destinations
  add constraint ntfy_destinations_label_nonempty
  check (length(btrim(label)) > 0) not valid;

alter table ntfy_destinations
  validate constraint ntfy_destinations_label_nonempty;

alter table ntfy_destinations
  add constraint ntfy_destinations_url_nonempty
  check (length(btrim(url)) > 0) not valid;

alter table ntfy_destinations
  validate constraint ntfy_destinations_url_nonempty;

alter table ntfy_destinations
  add constraint ntfy_destinations_topic_nonempty
  check (length(btrim(topic)) > 0) not valid;

alter table ntfy_destinations
  validate constraint ntfy_destinations_topic_nonempty;
...
alter table alert_rule_actions
  ...
  check (provider in ('telegram', 'webhook', 'email', 'discord', 'google_chat', 'ntfy')) not valid;
...
create index if not exists notification_intents_ntfy_claim_idx
  on notification_intents(provider, status, next_attempt_at, created_at)
  where provider = 'ntfy' and status in ('pending', 'failed');
