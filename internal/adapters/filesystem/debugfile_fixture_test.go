package filesystem_test

import (
	"bytes"
	"context"
	"errors"
	"io"
	"path/filepath"
	"testing"

	"github.com/ivanzakutnii/error-tracker/internal/adapters/filesystem"
	"github.com/ivanzakutnii/error-tracker/internal/app/debugfiles"
	"github.com/ivanzakutnii/error-tracker/internal/domain"
)

const breakpadFixture = "MODULE Linux x86_64 deadbeefcafef00ddeadbeefcafef00d libapp.so\n" +
	"FILE 0 src/app.c\n" +
	"FUNC 0 4 0 main\n" +
	"0 4 1 0\n"

func TestDebugFileFixtureRoundTripsAndDegrades(t *testing.T) {
	ctx := context.Background()
	root := t.TempDir()

	vault, vaultErr := filesystem.NewVault(filepath.Clean(root))
	if vaultErr != nil {
		t.Fatalf("vault: %v", vaultErr)
	}

	service, serviceErr := debugfiles.NewService(vault)
	if serviceErr != nil {
		t.Fatalf("service: %v", serviceErr)
	}

	organizationID, _ := domain.NewOrganizationID("11111111-1111-1111-1111-111111111111")
	projectID, _ := domain.NewProjectID("22222222-2222-2222-2222-222222222222")

	debugID, _ := domain.NewDebugIdentifier("deadbeefcafef00ddeadbeefcafef00d")
	fileName, _ := domain.NewDebugFileName("libapp.so.sym")
	identity, identityErr := domain.NewDebugFileIdentity(debugID, domain.DebugFileKindBreakpad(), fileName)
	if identityErr != nil {
		t.Fatalf("identity: %v", identityErr)
	}

	uploadResult := service.Upload(
		ctx,
		organizationID,
		projectID,
		identity,
		bytes.NewReader([]byte(breakpadFixture)),
	)
	stored, uploadErr := uploadResult.Value()
	if uploadErr != nil {
		t.Fatalf("upload: %v", uploadErr)
	}

	if stored.Size() != int64(len(breakpadFixture)) {
		t.Fatalf("unexpected size %d, want %d", stored.Size(), len(breakpadFixture))
	}

	getResult := service.Get(ctx, organizationID, projectID, identity)
	reader, getErr := getResult.Value()
	if getErr != nil {
		t.Fatalf("get: %v", getErr)
	}
	defer reader.Close()

	readBack, readErr := io.ReadAll(reader)
	if readErr != nil {
		t.Fatalf("read: %v", readErr)
	}

	if string(readBack) != breakpadFixture {
		t.Fatalf("payload mismatch:\nwant: %q\n got: %q", breakpadFixture, readBack)
	}

	listResult := service.List(ctx, organizationID, projectID)
	listings, listErr := listResult.Value()
	if listErr != nil {
		t.Fatalf("list: %v", listErr)
	}

	if len(listings) != 1 {
		t.Fatalf("expected 1 listing, got %d", len(listings))
	}

	if listings[0].ArtifactKey().Name().String() != identity.ArtifactName().String() {
		t.Fatalf(
			"unexpected artifact name %q, want %q",
			listings[0].ArtifactKey().Name().String(),
			identity.ArtifactName().String(),
		)
	}

	missingDebugID, _ := domain.NewDebugIdentifier("ffffffffffffffffffffffffffffffff")
	missingIdentity, _ := domain.NewDebugFileIdentity(missingDebugID, domain.DebugFileKindBreakpad(), fileName)
	missingResult := service.Get(ctx, organizationID, projectID, missingIdentity)
	if _, err := missingResult.Value(); !errors.Is(err, debugfiles.ErrDebugFileNotFound) {
		t.Fatalf("expected ErrDebugFileNotFound, got %v", err)
	}
}

func TestDebugFileFixtureRejectsUnsupportedPayload(t *testing.T) {
	ctx := context.Background()
	root := t.TempDir()

	vault, vaultErr := filesystem.NewVault(filepath.Clean(root))
	if vaultErr != nil {
		t.Fatalf("vault: %v", vaultErr)
	}

	service, serviceErr := debugfiles.NewService(vault)
	if serviceErr != nil {
		t.Fatalf("service: %v", serviceErr)
	}

	organizationID, _ := domain.NewOrganizationID("11111111-1111-1111-1111-111111111111")
	projectID, _ := domain.NewProjectID("22222222-2222-2222-2222-222222222222")

	debugID, _ := domain.NewDebugIdentifier("deadbeefcafef00ddeadbeefcafef00d")
	fileName, _ := domain.NewDebugFileName("libapp.so.sym")
	identity, _ := domain.NewDebugFileIdentity(debugID, domain.DebugFileKindBreakpad(), fileName)

	uploadResult := service.Upload(
		ctx,
		organizationID,
		projectID,
		identity,
		bytes.NewReader([]byte("zip archive or other unsupported payload")),
	)
	if _, err := uploadResult.Value(); !errors.Is(err, debugfiles.ErrUnsupportedDebugFile) {
		t.Fatalf("expected ErrUnsupportedDebugFile, got %v", err)
	}
}
