...
alter table discord_destinations
  add constraint discord_destinations_label_nonempty
  check (length(btrim(label)) > 0) not valid;

alter table discord_destinations
  validate constraint discord_destinations_label_nonempty;

alter table discord_destinations
  add constraint discord_destinations_url_nonempty
  check (length(btrim(url)) > 0) not valid;

alter table discord_destinations
  validate constraint discord_destinations_url_nonempty;
...
alter table alert_rule_actions
  ...
  check (provider in ('telegram', 'webhook', 'email', 'discord')) not valid;
...
create index if not exists notification_intents_discord_claim_idx
  on notification_intents(provider, status, next_attempt_at, created_at)
  where provider = 'discord' and status in ('pending', 'failed');
